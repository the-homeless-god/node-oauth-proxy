...
export const loginCallback = (_request: Request, response: Response): void => response.redirect(Routers.root)

export const getLoginMiddleware = (strategy: Strategy) => (
  request: Request,
  response: Response,
  next: NextFunction,
): RequestHandler => passport.authenticate(strategy.name, strategy.scopeContainer)(request, response, next)

export const initLoginCallbackRoute = (router: Router, strategy: Strategy): void => {
